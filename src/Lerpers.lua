...
do  --// Color3 HSV
    ...
    function lerpers.HSVColor3(origin, goal)
...
do  --// NumberRange
    ...
    function lerpers.NumberRange(origin, goal)
...
do  --// ColorSequence
    ...
    function lerpers.NumberSequence(origin, goal)
        ...
        local goalKeypointsHash = {}
        local deltaKeypoints = {
            ColorSequence.new(0, originKeypoints[1].Value - goalKeypoints[1].Value)
        }
        ...
        repeat
            ...
            if originTime > goalTime then
                ...
                if not originKeypointsHash[goalTime] then
                    ...
                    table.insert(originKeypoints, index, ColorSequenceKeypoint.new(goalTime, originValue))
                end
                ...
                if not goalKeypointsHash[originTime] then
                    ...
                    table.insert(goalKeypoints, index+1, ColorSequenceKeypoint.new(originTime, goalValue))
                end
                
                deltaKeypoints[index] = ColorSequence.new(goalTime, goalValue - originValue)
            ...
            elseif originTime < goalTime then
                ...
                if not goalKeypointsHash[originTime] then
                    ...
                    table.insert(goalKeypoints, index, ColorSequenceKeypoint.new(originTime, goalValue))
                end
                ...
                if not originKeypointsHash[goalTime] then
                    ...
                    table.insert(originKeypoints, index+1, ColorSequenceKeypoint.new(goalTime, originValue))
                end
                
                deltaKeypoints[index] = ColorSequence.new(originTime, goalValue - originValue)
            else
...
do  --// Default
    local function lerp(origin, goal, multiplier)
        ...
        return if multiplier > .5 then goal else origin
    end
    
    function lerpers.default(origin, goal)
...
--// End
export type lerper_init = <T, D1, D2>(origin: T, goal: T) -> (lerper_updt<T, D1, D2>, D1, D2)
export type lerper_updt<T, D1, D2> = (D1, D2, number) -> T

return lerpers :: { [string]: lerper_init }